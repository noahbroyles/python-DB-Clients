...
class Database:
    ...
    def __init__(self, server: str = DBHOST, port: int = 1433, database: str = DATABASE, username: str = DBUSER, password: str = DBPASSWD, driver: str = 'FreeTDS', charset: str = 'UTF-8', extra_conn_string: str = 'TDS_Version=8.0'):

        self._connection_string = 'DRIVER={' + driver + '};' + f"SERVER={server};PORT={port};DATABASE={database};UID={username};PWD={password};CHARSET={charset};{extra_conn_string}"
...
Parameterizes statement and runs in the database. Use for INSERT, UPDATE, and DROP commands where no results are expected to be returned.
    ...
    def execute_many(self, sqlStmt: str, multipleParams: list, fast: bool = False, commit: bool = False, convert_blanks_to_nulls: bool = True):
        ...
        if convert_blanks_to_nulls:
            multipleParams = [[p if p != "" else None for p in l] for l in multipleParams]

        cursor.executemany(sqlStmt, multipleParams)
